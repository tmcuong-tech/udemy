...
Adapted for TGS-2024048316 — CompTIA Certified Linux+ Training (XK0-006).
"""
import os
...
# CompTIA Linux+ brand-adjacent palette (kept professional / print-safe)
BRAND = RGBColor(0xC8, 0x10, 0x2E)   # CompTIA red
...
def _line(doc, text, size, bold=False, color=DARK, before=0, after=4, caps=False):
    ...
    r = p.add_run(text.upper() if caps else text); r.bold = bold; r.font.size = Pt(size)
...
def style_headings(doc):
    ...
    specs = {"Title": (24, DARK, True), "Heading 1": (16, BRAND, True),
             ...
             "Normal": (11, DARK, False)}
    for name, (sz, col, bold) in specs.items():
        try:
            st = doc.styles[name]; st.font.size = Pt(sz); st.font.color.rgb = col
            st.font.bold = bold; st.font.name = "Arial"
        except KeyError:
            pass
...
def add_cover_page(doc, kind, title, version, conducted_by=ORG,
                   org_logo=None, course_logo=None, tgs=TGS):
    ...
    _img(doc, course_logo, 1.4)                            # CompTIA Linux+ course logo
    _line(doc, title, 20, bold=True, color=DARK, before=6, after=2)
...
def add_version_control(doc, rows):
    ...
    r.bold = True; r.font.size = Pt(12); r.font.color.rgb = DARK; r.font.name = "Arial"
    ...
    for i, htext in enumerate(hdr):
        ...
        rr.font.name = "Arial"; _shade_cell(hr[i], "C8102E")
    for row in rows:
        cells = t.add_row().cells
        for i, val in enumerate(row):
            cells[i].text = ""; rr = cells[i].paragraphs[0].add_run(str(val))
...
def add_page_numbers(doc):
    ...
    r = p.add_run("Page "); r.font.size = Pt(9); r.font.color.rgb = GREY; r.font.name = "Arial"
    _field(p, "PAGE", "1").font.size = Pt(9)
    r2 = p.add_run(" of "); r2.font.size = Pt(9); r2.font.color.rgb = GREY; r2.font.name = "Arial"
    ...
    sr.font.size = Pt(7.5); sr.font.color.rgb = GREY; sr.font.name = "Arial"
