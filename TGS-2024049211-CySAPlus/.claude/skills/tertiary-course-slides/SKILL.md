...
description: Generate a highly professional, all-white-theme WSQ course slide deck (python-pptx) for Tertiary Infotech Academy. Produces a cover page with course title + course & Tertiary logos + UEN, admin slides (digital attendance/TRAQOM, About the Trainer, Ground Rules, Lesson Plan, Learning Outcomes, Assessment), course key-concept slides, per-activity overview + workflow screenshot + step-by-step slides, and lunch/tea-break dividers. Use when creating or updating course/training slides for a Tertiary Infotech WSQ course.
---
...
- **Cover slide**: course title, the **course logo (CompTIA CySA+ badge)** (top-right) and **Tertiary Infotech Academy logo** (top-left), `WSQ Course Code: TGS-XXXX`, `Conducted by Tertiary Infotech Academy Pte Ltd`, `UEN 201200696W`.
...
- **course key concepts**: What is n8n, nodes, triggers & actions, execution modes, data/JSON/expressions, pin data, code/edit-fields, IF/Switch, split out, merge, sub-workflows; AI agents (LLM/memory/tools/system prompt); RAG (embeddings/vector store); API & HTTP request; webhooks & auth; security/guardrails.
...
- **Cover page** with: the **Course Title**; the **course logo (CompTIA CySA+ badge)** and the **Tertiary Infotech Academy Pte Ltd logo**; `WSQ Course Code: TGS-XXXX`; `Conducted by Tertiary Infotech Academy Pte Ltd`; `UEN: 201200696W`; and a **Version** number. (Logos live in `.claude/skills/tertiary-course-slides/assets/`: `tertiary-infotech-logo.png`, `cysa-course-logo.png`.)
